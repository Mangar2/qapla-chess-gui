...
void ChatbotLlmChat::ensureController() {
    ...
    std::string systemPrompt = std::format(
        ...
        "UCI options: get_engine_details lists what one engine supports (names, defaults, "
        "ranges) -- read it before set_engine_options, never invent an option name. "
        "set_engine_options needs target: an engine selected for a run is a copy, so target="
        "sprt/tournament/epd changes that run only and target=catalog changes the installed "
        "engine future selections start from. Select engines first, set options after.\n"
