...
void ChatbotLlmChat::ensureController() {
    ...
    std::string systemPrompt = std::format(
        ...
        "required to start.\n"
        "show_result (type=tournament/sprt/epd) renders results as table in chat directly -- "
        "after calling, don't repeat data as text, just short confirmation. Never state/guess "
        "score/Elo/result numbers yourself -- only source of truth is this tool's own response. "
        "Guessing = serious error.\n"
