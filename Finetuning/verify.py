"""Blind verification of the finetuning dataset (finetuning-001.json by default).

Deliberately knows nothing about how the records were produced -- it re-derives every
rule from the system prompt and the tool schemas and reports violations. Run it against
the finished file, not against the authoring modules.
"""
import json
import re
import sys
...
DATA = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "finetuning-001.json"
